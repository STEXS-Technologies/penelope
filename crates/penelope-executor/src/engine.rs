...
use penelope_domain::{
    ActionId, ContentDigest, DefinitionId, DefinitionVersion, InputId, LogicalTime,
    MAX_ACTION_ATTEMPTS_PER_STEP, MAX_DEFINITION_STEPS, ManualReview, ProcessAction,
    ProcessActionKind, ProcessId, ProcessOutcome, ProcessOutcomeFact, ProcessOutcomeKind,
    ProcessScope, ReviewId, StepId, TenantId,
};
use penelope_ports::{ManualReviewResolution, TimerSchedule};
...
/// [`RetryJitterSeed`] in the scheduling event so replay has exactly the
/// same result.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
pub struct RetryJitterSeed(ContentDigest);

impl RetryJitterSeed {
...
impl RetryPolicy {
    ...
    pub fn retry_due_at(
        ...
        now: LogicalTime,
        retry_ordinal: u32,
        jitter_seed: RetryJitterSeed,
    ) -> Result<Option<LogicalTime>, EngineError> {
        ...
        Ok(Some(LogicalTime(due_at)))
    }
}

fn deterministic_jitter(
    policy: RetryJitter,
    RetryJitterSeed(ContentDigest(seed)): RetryJitterSeed,
    retry_ordinal: u32,
) -> u64 {
...
impl StepPlan {
    ...
    pub const fn with_compensation(mut self, compensation: CompensationPlan) -> Self {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct LinearSagaDefinition {
...
pub enum GraphTransitionOutcome {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ProcessGraphDefinition {
    /// Stable immutable definition identity.
    pub definition_id: DefinitionId,
...
impl ProcessGraphDefinition {
    ...
    pub fn validate(&self) -> Result<(), GraphDefinitionError> {
        ...
        for transition in &self.transitions {
            ...
            } else if transition.on == GraphTransitionOutcome::RetryableFailure {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct LinearSagaProjection {
    ...
    pub retry_due_at: Option<LogicalTime>,
...
/// produced it.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ActionResultObservation {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct RetryTimerScheduleRequest {
    /// Correlated retryable result for the active effect action.
    pub observation: ActionResultObservation,
    ...
    pub jitter_seed: RetryJitterSeed,
    /// Optional inclusive logical deadline beyond which a retry is unsafe.
    pub retry_deadline: Option<LogicalTime>,
}

impl RetryTimerScheduleRequest {
    ...
    const fn with_optional_deadline(mut self, deadline: Option<LogicalTime>) -> Self {
...
pub enum LinearSagaInput {
    ...
    RetryTimerScheduled {
        ...
        #[serde(default)]
        retry_deadline: Option<LogicalTime>,
    },
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct LinearSagaEventEnvelope {
    /// Zero-based, contiguous event sequence for one process.
    pub sequence: u64,
    /// The event recorded at `sequence`.
    pub event: LinearSagaEvent,
}

impl ActionResultObservation {
    ...
    pub const fn retryable_failure(action_id: ActionId) -> Self {
        Self {
            action_id,
            result: ActionResult::RetryableFailure,
        }
    }
    ...
    pub const fn terminal_failure(action_id: ActionId) -> Self {
        Self {
            action_id,
            result: ActionResult::TerminalFailure,
        }
    }
}

impl LinearSagaInput {
    ...
    #[must_use]
    pub fn to_event(&self, definition: &LinearSagaDefinition) -> LinearSagaEvent {
        match self {
            Self::Start {
                input_id,
                action_id,
            } => LinearSagaEvent::started(definition, input_id.clone(), action_id.clone()),
            Self::ActionResult {
                input_id,
                observation,
                next_action_id,
            ...
            Self::RetryTimerScheduled {
                ...
                jitter_seed,
                retry_deadline,
            } => LinearSagaEvent::RetryTimerScheduled {
                input_id: input_id.clone(),
                observation: observation.clone(),
                next_action_id: next_action_id.clone(),
...
impl LinearSagaEvent {
    ...
    ///
    /// The caller combines these with [`SagaDecision::planned_outcome_kinds`]
    /// and persists the resulting ordered list atomically with the decision.
    #[must_use]
    pub const fn observed_outcome_kinds(&self) -> &'static [ProcessOutcomeKind] {
        match self {
            Self::Started { .. } => &[
            ...
            Self::ActionResultObserved { observation, .. } => match observation.result {
                ActionResult::Succeeded => &[
                    ProcessOutcomeKind::InputAccepted,
                    ProcessOutcomeKind::ActionSucceeded,
                ],
                ActionResult::RetryableFailure | ActionResult::TerminalFailure => &[
                    ProcessOutcomeKind::InputAccepted,
                    ProcessOutcomeKind::ActionFailed,
                ],
                ActionResult::Unknown => &[
                    ProcessOutcomeKind::InputAccepted,
                    ProcessOutcomeKind::ActionOutcomeUnknown,
                ],
            },
            Self::RetryTimerScheduled { .. } => &[
                ProcessOutcomeKind::InputAccepted,
            ...
            Self::RetryTimerFired { .. } => &[
                ProcessOutcomeKind::InputAccepted,
                ProcessOutcomeKind::TimerFired,
            ],
            Self::ManualResolutionApplied { .. } => &[
                ProcessOutcomeKind::InputAccepted,
                ProcessOutcomeKind::ManualResolutionApplied,
            ],
        }
    }
}
...
pub struct SagaDecision {
    ...
    pub projection: LinearSagaProjection,
...
impl SagaDecision {
    ...
    pub fn retry_timer_schedule(&self) -> Option<TimerSchedule> {
        Some(TimerSchedule {
    ...
    ) -> Result<ManualReview, EngineError> {
        if self.projection.status != SagaStatus::Escalated {
    ...
    #[must_use]
    pub const fn planned_outcome_kinds(&self) -> &'static [ProcessOutcomeKind] {
        if self.next_action.is_some() {
            if self.projection.retry_due_at.is_some() {
                return &[
                    ProcessOutcomeKind::RetryScheduled,
                    ProcessOutcomeKind::ActionPlanned,
                ];
            }
            if matches!(self.projection.status, SagaStatus::Compensating) {
                return &[ProcessOutcomeKind::CompensationPlanned];
            }
            return &[ProcessOutcomeKind::ActionPlanned];
        }
        match self.projection.status {
            SagaStatus::Completed => &[ProcessOutcomeKind::Completed],
            SagaStatus::Cancelled => &[ProcessOutcomeKind::Cancelled],
            SagaStatus::Compensated => &[ProcessOutcomeKind::Compensated],
            SagaStatus::Escalated => &[ProcessOutcomeKind::Escalated],
    ...
    pub fn build_required_outcomes(
        ...
        for fact in facts
            .iter()
            .filter(|fact| matches!(fact.kind, ProcessOutcomeKind::InputAccepted))
        {
            let Some(input_id) = event.input_id() else {
                return Err(EngineError::OutcomeInputCausationMismatch);
            };
            if fact.causation_id != penelope_domain::CausationId::Input(input_id.clone()) {
                return Err(EngineError::OutcomeInputCausationMismatch);
            }
        }
        facts
            ...
            .map(|(index, fact)| {
                ...
                Ok(ProcessOutcome::new(scope.clone(), sequence, fact.clone()))
            })
            .collect()
    }
}
...
pub fn decide(
    definition: &LinearSagaDefinition,
    projection: Option<&LinearSagaProjection>,
    tenant_id: TenantId,
    process_id: ProcessId,
    input: &LinearSagaInput,
) -> Result<SagaDecision, EngineError> {
    match input {
        LinearSagaInput::Start { action_id, .. } => {
            if projection.is_some() {
                return Err(EngineError::StartWithProjection);
            }
            start(definition, tenant_id, process_id, action_id.clone())
        }
        LinearSagaInput::ActionResult {
            observation,
            next_action_id,
            ..
        } => apply_action_result(
        ...
        LinearSagaInput::RetryTimerScheduled {
            observation,
            next_action_id,
            observed_at,
            jitter_seed,
        ...
        LinearSagaInput::RetryTimerFired {
            timer_action_id,
            fired_at,
            next_action_id,
            ..
        ...
        LinearSagaInput::ManualResolution {
            resolution,
            next_action_id,
            ..
...
pub fn start(
    ...
    definition.validate()?;
    let projection = LinearSagaProjection {
        definition_id: definition.definition_id.clone(),
        definition_version: definition.definition_version.clone(),
        definition_digest: definition.definition_digest,
        next_step_index: 0,
...
pub fn apply_action_result(
    definition: &LinearSagaDefinition,
    projection: &LinearSagaProjection,
    tenant_id: TenantId,
    process_id: ProcessId,
    observation: &ActionResultObservation,
    next_action_id: Option<ActionId>,
    ...
    if projection.status != SagaStatus::Running && projection.status != SagaStatus::Compensating {
    ...
    match projection.status {
        SagaStatus::Running => apply_forward_result(
            definition,
            projection,
            tenant_id,
            process_id,
        ...
        SagaStatus::Compensating => apply_compensation_result(
            definition,
            projection,
            tenant_id,
            process_id,
        ...
        | SagaStatus::Compensated => Err(EngineError::TerminalProjection),
    }
}
...
pub fn schedule_retry_timer(
    ...
    tenant_id: TenantId,
    process_id: ProcessId,
    request: &RetryTimerScheduleRequest,
) -> Result<SagaDecision, EngineError> {
    definition.validate()?;
    if projection.definition_id != definition.definition_id
        || projection.definition_version != definition.definition_version
    ...
    if request.observation.result != ActionResult::RetryableFailure {
    ...
    let timer = LinearSagaProjection {
        current_attempt,
...
pub fn fire_retry_timer(
    ...
    let retry = LinearSagaProjection {
        retry_due_at: None,
        active_action_id: Some(action_id.clone()),
        issued_action_ids: issued_ids_after(projection, action_id),
        ..projection.clone()
...
pub fn apply_manual_resolution(
    ...
    if projection.status != SagaStatus::Escalated {
    ...
    match resolution {
        ManualReviewResolution::RetryAction => {
            let action_id = fresh_action_id(projection, next_action_id)?;
            let resumed = LinearSagaProjection {
                active_action_id: Some(action_id.clone()),
                issued_action_ids: issued_ids_after(projection, action_id),
                status: SagaStatus::Running,
                ..projection.clone()
        ...
        ManualReviewResolution::Compensate => {
            ...
            let action_id = fresh_action_id(projection, next_action_id)?;
            let compensating = LinearSagaProjection {
                current_attempt: 0,
                active_action_id: Some(action_id.clone()),
                issued_action_ids: issued_ids_after(projection, action_id),
        ...
        ManualReviewResolution::Cancel => Ok(SagaDecision {
            projection: terminal_projection(
                projection,
                projection.next_step_index,
        ...
        ManualReviewResolution::Escalate => Ok(SagaDecision {
...
fn apply_forward_result(
    ...
    match result {
        ...
        ActionResult::TerminalFailure => begin_compensation(
            definition,
            projection,
            tenant_id,
            process_id,
        ...
        ActionResult::Unknown => Ok(SagaDecision {
            projection: terminal_projection(
                projection,
                projection.next_step_index,
                projection.compensable_step_indices.clone(),
...
fn apply_compensation_result(
    ...
    match result {
        ...
        ActionResult::TerminalFailure | ActionResult::Unknown => Ok(SagaDecision {
            projection: terminal_projection(
                projection,
                projection.next_step_index,
                projection.compensable_step_indices.clone(),
...
fn compensation_for(
    definition: &LinearSagaDefinition,
    projection: &LinearSagaProjection,
) -> Result<CompensationPlan, EngineError> {
...
fn issued_ids_after(projection: &LinearSagaProjection, action_id: ActionId) -> Vec<ActionId> {
...
pub fn replay(
    ...
    tenant_id: &TenantId,
    process_id: &ProcessId,
    events: &[LinearSagaEvent],
) -> Result<SagaDecision, EngineError> {
    let mut decision: Option<SagaDecision> = None;
...
pub fn replay_ordered(
    ...
    tenant_id: &TenantId,
    process_id: &ProcessId,
    envelopes: &[LinearSagaEventEnvelope],
) -> Result<SagaDecision, EngineError> {
...
fn action_for(
    ...
    tenant_id: TenantId,
    process_id: ProcessId,
) -> Result<ProcessAction, EngineError> {
    let (step_id, action_kind, payload_digest) = if projection.retry_due_at.is_some() {
        ...
        (
            step.step_id.clone(),
            ProcessActionKind::Timer,
            step.payload_digest,
        )
    } else if projection.status == SagaStatus::Compensating {
        let step_index = projection
            .compensable_step_indices
            .last()
            .ok_or(EngineError::InvalidProjection)?;
...
mod tests {
    use super::*;
    use penelope_domain::{CausationId, OutcomeActor, OutcomeId, ProcessOutcomeFact};
    ...
    fn definition() -> LinearSagaDefinition {
        let retry_policy = RetryPolicy::new(NonZeroU32::new(2).unwrap());
        LinearSagaDefinition {
    ...
    fn linear_definition_rejects_duplicate_and_oversized_steps() {
        ...
        let duplicate = LinearSagaDefinition::new(
            id("def_trade"),
            id("dfv_one"),
            ContentDigest([99; 32]),
            vec![
                StepPlan::canonical_command(id("stp_lock"), ContentDigest([1; 32]), policy),
                StepPlan::canonical_command(id("stp_lock"), ContentDigest([2; 32]), policy),
            ],
        );
        assert_eq!(
        ...
        let oversized = LinearSagaDefinition::new(
            id("def_trade"),
            id("dfv_one"),
            ContentDigest([99; 32]),
            vec![
                StepPlan::canonical_command(id("stp_limit"), ContentDigest([3; 32]), policy);
        ...
        let excessive_retries = LinearSagaDefinition::new(
            id("def_trade"),
            id("dfv_one"),
            ContentDigest([99; 32]),
    ...
    fn graph_definition_validates_references_reachability_and_terminal_edges() {
        let policy = RetryPolicy::no_retry();
        let graph = ProcessGraphDefinition {
            definition_id: id("def_graph"),
            definition_version: id("dfv_one"),
            definition_digest: ContentDigest([4; 32]),
            steps: vec![
                StepPlan::canonical_command(id("stp_entry"), ContentDigest([1; 32]), policy),
                StepPlan::canonical_command(id("stp_next"), ContentDigest([2; 32]), policy),
            ],
            ...
            transitions: vec![
                ...
                GraphTransition {
                    ...
                    on: GraphTransitionOutcome::TerminalFailure,
                    to_step: None,
                },
            ],
        ...
        missing_destination.transitions.first_mut().unwrap().on =
            GraphTransitionOutcome::RetryableFailure;
        ...
        unreachable.steps.push(StepPlan::canonical_command(
            id("stp_orphan"),
            ContentDigest([3; 32]),
            policy,
        ));
    ...
    fn observation(action: &ProcessAction, result: ActionResult) -> ActionResultObservation {
    ...
    fn outcome_fact(outcome_id: OutcomeId, kind: ProcessOutcomeKind) -> ProcessOutcomeFact {
    ...
    fn input_accepted_fact(outcome_id: OutcomeId, input_id: InputId) -> ProcessOutcomeFact {
        ProcessOutcomeFact::new(
            outcome_id,
            CausationId::Input(input_id),
            OutcomeActor::System,
            LogicalTime(0),
            ProcessOutcomeKind::InputAccepted,
            ContentDigest([0; 32]),
    ...
    fn outcome_plan_requires_every_observed_and_planned_lifecycle_fact_in_order() {
        ...
        let event = LinearSagaEvent::started(&definition, id("inp_start"), id("act_lock"));
        ...
        let outcomes = decision
            .build_required_outcomes(
                ...
                &[
                    ...
                    outcome_fact(id("out_started"), ProcessOutcomeKind::Started),
                    outcome_fact(id("out_planned"), ProcessOutcomeKind::ActionPlanned),
                ],
            )
            .unwrap();
        assert_eq!(
        ...
        assert_eq!(
            decision.build_required_outcomes(
                ...
                0,
                &[
                    outcome_fact(id("out_wrong_started"), ProcessOutcomeKind::Started),
                    outcome_fact(id("out_wrong_planned"), ProcessOutcomeKind::ActionPlanned),
                ],
            ),
        ...
        assert_eq!(
            decision.build_required_outcomes(
                ...
                &[
                    ...
                    outcome_fact(id("out_overflow_started"), ProcessOutcomeKind::Started),
                    outcome_fact(
    ...
    fn external_saga_inputs_always_require_an_accepted_input_record() {
        let event = LinearSagaEvent::ActionResultObserved {
            ...
            observation: ActionResultObservation::succeeded(id("act_lock")),
        ...
        let timer_event = LinearSagaEvent::RetryTimerFired {
            input_id: id("inp_timer_fire"),
            timer_action_id: id("act_timer"),
            fired_at: LogicalTime(1),
        ...
        let resolution_event = LinearSagaEvent::ManualResolutionApplied {
    ...
    fn outcome_batch_binds_accepted_input_to_the_event_input_identity() {
        ...
        let decision = apply_action_result(
            ...
            id("tnt_game"),
            id("prc_trade"),
            &ActionResultObservation::succeeded(action_id),
            Some(id("act_settle")),
        )
        .unwrap();
        let scope = ProcessScope::new(
        ...
        let wrong_input_fact = ProcessOutcomeFact::new(
            id("out_wrong_input"),
            CausationId::Input(id("inp_other")),
            OutcomeActor::System,
            LogicalTime(1),
            ProcessOutcomeKind::InputAccepted,
            ContentDigest([0; 32]),
        ...
        assert_eq!(
            decision.build_required_outcomes(
                ...
                &[
                    wrong_input_fact,
                    outcome_fact(id("out_result"), ProcessOutcomeKind::ActionSucceeded),
                    outcome_fact(id("out_next"), ProcessOutcomeKind::ActionPlanned),
                ],
            ),
    ...
    fn escalated_decision_builds_a_scope_pinned_manual_review_request() {
        ...
        let escalated = apply_action_result(
            ...
            id("tnt_game"),
            id("prc_trade"),
            &ActionResultObservation::unknown(action_id),
            None,
        )
        .unwrap();
        let scope = ProcessScope::new(
        ...
        assert_eq!(request.expires_at, Some(LogicalTime(9)));
    ...
    fn success_plans_ordered_steps_then_completes() {
        ...
        let second = apply_action_result(
            ...
            id("tnt_game"),
            id("prc_trade"),
            &observation(first.next_action.as_ref().unwrap(), ActionResult::Succeeded),
            Some(id("act_settle")),
        )
        .unwrap();
        assert_eq!(
        ...
        .unwrap();
        assert_eq!(complete.projection.status, SagaStatus::Completed);
    }
    ...
    fn decide_routes_typed_start_and_result_inputs_without_side_effects() {
        ...
        let advanced = decide(
            ...
            &LinearSagaInput::ActionResult {
                ...
                observation: ActionResultObservation::succeeded(
    ...
    fn input_to_event_preserves_the_exact_immutable_input_identity() {
        let input = LinearSagaInput::ActionResult {
    ...
    fn retry_schedule_input_preserves_the_jitter_seed_in_its_replay_event() {
        let seed = RetryJitterSeed::from_digest(ContentDigest([23; 32]));
        let input = LinearSagaInput::RetryTimerScheduled {
            input_id: id("inp_retry_failure"),
            observation: ActionResultObservation::retryable_failure(id("act_lock")),
            next_action_id: Some(id("act_retry_timer")),
            observed_at: LogicalTime(100),
            jitter_seed: seed,
            retry_deadline: None,
        };
        assert!(matches!(
    ...
    fn retry_backoff_is_deterministic_exponential_and_capped() {
        let backoff =
            RetryBackoff::new(NonZeroU64::new(10).unwrap(), NonZeroU64::new(40).unwrap()).unwrap();
        let policy = RetryPolicy::new(NonZeroU32::new(5).unwrap()).with_backoff(backoff);
        let seed = RetryJitterSeed::from_digest(ContentDigest([0; 32]));
        assert_eq!(
            policy.retry_due_at(LogicalTime(100), 0, seed).unwrap(),
            Some(LogicalTime(110))
        );
        assert_eq!(
            policy.retry_due_at(LogicalTime(100), 1, seed).unwrap(),
            Some(LogicalTime(120))
        );
        assert_eq!(
            policy.retry_due_at(LogicalTime(100), 2, seed).unwrap(),
            Some(LogicalTime(140))
        );
        assert_eq!(
            policy.retry_due_at(LogicalTime(100), 31, seed).unwrap(),
            Some(LogicalTime(140))
        );
    }
    ...
    fn retry_jitter_is_seeded_bounded_and_replayable() {
        let backoff = RetryBackoff::new(NonZeroU64::new(10).unwrap(), NonZeroU64::new(40).unwrap())
            .unwrap()
            .with_jitter(RetryJitter {
                max_jitter_millis: NonZeroU64::new(7).unwrap(),
            });
        let policy = RetryPolicy::new(NonZeroU32::new(3).unwrap()).with_backoff(backoff);
        let seed = RetryJitterSeed::from_digest(ContentDigest([17; 32]));
        let first = policy.retry_due_at(LogicalTime(100), 0, seed).unwrap();
        let replayed = policy.retry_due_at(LogicalTime(100), 0, seed).unwrap();
        assert_eq!(first, replayed);
        assert!(matches!(first, Some(LogicalTime(due_at)) if (110..117).contains(&due_at)));
    }

    #[test]
    fn retry_deadline_allows_its_boundary_and_escalates_after_it() {
        let backoff =
            RetryBackoff::new(NonZeroU64::new(10).unwrap(), NonZeroU64::new(10).unwrap()).unwrap();
        let definition = LinearSagaDefinition::new(
            ...
            vec![StepPlan::canonical_command(
                id("stp_settle"),
                ContentDigest([1; 32]),
                RetryPolicy::new(NonZeroU32::new(2).unwrap()).with_backoff(backoff),
            )],
        );
        ...
        .unwrap();
        let observation = ActionResultObservation::retryable_failure(id("act_settle"));
        let request = RetryTimerScheduleRequest::new(
            observation.clone(),
            Some(id("act_timer")),
            LogicalTime(100),
            RetryJitterSeed::from_digest(ContentDigest([0; 32])),
        );
        let allowed = schedule_retry_timer(
            ...
            tenant_id.clone(),
            process_id.clone(),
            &request.clone().with_deadline(LogicalTime(110)),
        )
        .unwrap();
        assert_eq!(allowed.projection.retry_due_at, Some(LogicalTime(110)));
        ...
        let expired = schedule_retry_timer(
            ...
            tenant_id.clone(),
            process_id.clone(),
            &request.with_deadline(LogicalTime(109)),
        )
        .unwrap();
        assert_eq!(expired.projection.status, SagaStatus::Escalated);
        assert!(expired.next_action.is_none());

        let events = [
            LinearSagaEvent::started(&definition, id("inp_start"), id("act_settle")),
            LinearSagaEvent::RetryTimerScheduled {
                input_id: id("inp_failure"),
                observation,
                next_action_id: Some(id("act_timer")),
                observed_at: LogicalTime(100),
                jitter_seed: RetryJitterSeed::from_digest(ContentDigest([0; 32])),
                retry_deadline: Some(LogicalTime(109)),
            },
        ];
        assert_eq!(
    ...
    fn retry_backoff_rejects_invalid_bounds_and_due_time_overflow() {
        let invalid = RetryBackoff::new(NonZeroU64::new(2).unwrap(), NonZeroU64::new(1).unwrap());
        assert_eq!(invalid.unwrap_err(), EngineError::InvalidRetryBackoff);

        let backoff =
            RetryBackoff::new(NonZeroU64::new(1).unwrap(), NonZeroU64::new(1).unwrap()).unwrap();
        let policy = RetryPolicy::new(NonZeroU32::MIN).with_backoff(backoff);
        assert_eq!(
            policy.retry_due_at(
                LogicalTime(u64::MAX),
                0,
    ...
    fn retry_backoff_plans_replayable_due_timer_and_rejects_early_or_duplicate_fire() {
        let backoff =
            RetryBackoff::new(NonZeroU64::new(10).unwrap(), NonZeroU64::new(100).unwrap())
                .unwrap()
                .with_jitter(RetryJitter {
                    max_jitter_millis: NonZeroU64::new(7).unwrap(),
                });
        let definition = LinearSagaDefinition::new(
            ...
            vec![StepPlan::canonical_command(
                id("stp_lock"),
                ContentDigest([1; 32]),
                RetryPolicy::new(NonZeroU32::new(3).unwrap()).with_backoff(backoff),
            )],
        );
        ...
        let jitter_seed = RetryJitterSeed::from_digest(ContentDigest([17; 32]));
        ...
        assert_eq!(timer.kind, ProcessActionKind::Timer);
        assert!(matches!(due_at, LogicalTime(value) if (110..117).contains(&value)));
        assert_eq!(
            apply_action_result(
                &definition,
                ...
                tenant_id.clone(),
                process_id.clone(),
                &ActionResultObservation::succeeded(timer.action_id.clone()),
                Some(id("act_wrong")),
            )
            .unwrap_err(),
        ...
        let events = [
            LinearSagaEvent::started(&definition, id("inp_start"), id("act_first")),
            LinearSagaEvent::RetryTimerScheduled {
                input_id: id("inp_retry_failure"),
                observation: failure,
                next_action_id: Some(id("act_retry_timer")),
    ...
    fn authorized_manual_resolution_replays_compensation_or_cancellation() {
        ...
        let definition = LinearSagaDefinition::new(
            ...
            vec![
                ...
                StepPlan::canonical_command(id("stp_settle"), ContentDigest([2; 32]), policy),
            ],
        );
        let tenant_id = id::<TenantId>("tnt_market");
        let process_id = id::<ProcessId>("prc_trade");
        ...
        let escalated = apply_action_result(
            ...
            tenant_id.clone(),
            process_id.clone(),
            &observation(settle.next_action.as_ref().unwrap(), ActionResult::Unknown),
            None,
        )
        .unwrap();
        assert_eq!(escalated.projection.status, SagaStatus::Escalated);
        ...
        .unwrap();
        assert_eq!(compensation.projection.status, SagaStatus::Compensating);
        assert_eq!(
        ...
        let cancelled = apply_manual_resolution(
            ...
            tenant_id.clone(),
            process_id.clone(),
            ManualReviewResolution::Cancel,
            None,
        )
        .unwrap();
        assert_eq!(cancelled.projection.status, SagaStatus::Cancelled);
        assert_eq!(
            apply_manual_resolution(
                &definition,
                &settle.projection,
        ...
        let events = [
            LinearSagaEvent::started(&definition, id("inp_start"), id("act_lock")),
            LinearSagaEvent::ActionResultObserved {
                input_id: id("inp_lock_result"),
                observation: ActionResultObservation::succeeded(id("act_lock")),
                next_action_id: Some(id("act_settle")),
            ...
            LinearSagaEvent::ActionResultObserved {
                ...
                observation: ActionResultObservation::unknown(id("act_settle")),
                next_action_id: None,
            },
            LinearSagaEvent::ManualResolutionApplied {
    ...
    fn unknown_result_escalates_instead_of_blind_retry() {
        ...
        let decision = apply_action_result(
            ...
            id("tnt_game"),
            id("prc_trade"),
            &observation(started.next_action.as_ref().unwrap(), ActionResult::Unknown),
            None,
        )
        .unwrap();
    ...
    fn exhausted_retry_policy_escalates_without_a_new_action() {
        let definition = LinearSagaDefinition {
            ...
            definition_version: id("dfv_one"),
            definition_digest: ContentDigest([99; 32]),
            steps: vec![StepPlan::canonical_command(
                id("stp_lock"),
                ContentDigest([1; 32]),
    ...
    fn known_terminal_failure_compensates_succeeded_steps_in_lifo_order() {
        ...
        let definition = LinearSagaDefinition {
            ...
            steps: vec![
                ...
                StepPlan::canonical_command(id("stp_settle"), ContentDigest([3; 32]), policy),
            ],
        };
        let tenant_id = id::<TenantId>("tnt_game");
        let process_id = id::<ProcessId>("prc_trade");
        ...
        let unlock_b = apply_action_result(
            ...
            &observation(
                ...
                ActionResult::TerminalFailure,
            ),
            Some(id("act_unlock_b")),
        )
        .unwrap();
        assert_eq!(unlock_b.projection.status, SagaStatus::Compensating);
        assert_eq!(
        ...
        .unwrap();
        assert_eq!(compensated.projection.status, SagaStatus::Compensated);
    ...
    fn crash_after_every_trade_event_replays_to_the_same_compensated_state() {
        ...
        let events = vec![
            ...
            LinearSagaEvent::ActionResultObserved {
                ...
                observation: ActionResultObservation::succeeded(id("act_unlock")),
                next_action_id: None,
            },
        ];
        ...
        assert_eq!(final_decision.projection.status, SagaStatus::Compensated);
    ...
    proptest! {
        ...
        fn retry_attempt_is_monotonic_within_the_bounded_retry_policy(
            ...
            step.retry_policy = RetryPolicy::new(
        ...
        fn live_transitions_and_event_replay_converge(
            ...
            .unwrap();
            let mut events = vec![LinearSagaEvent::started(
                &definition,
            ...
            for code in result_codes {
                ...
                if matches!(result, ActionResult::Succeeded | ActionResult::RetryableFailure)
                ...
                events.push(LinearSagaEvent::ActionResultObserved {
                    input_id,
                    observation: observation.clone(),
                    next_action_id: next_action_id.clone(),
                });
    ...
    fn replay_rebuilds_the_final_projection_from_immutable_events() {
        ...
        let replayed = replay(
            ...
            &[
                ...
                LinearSagaEvent::ActionResultObserved {
                    input_id: id("inp_settle_result"),
                    observation: ActionResultObservation::succeeded(id("act_settle")),
                    next_action_id: None,
                },
            ],
        )
        .unwrap();
        assert_eq!(replayed.projection.status, SagaStatus::Completed);
    ...
    fn replay_rejects_a_second_start_record() {
        ...
        let error = replay(
            ...
            &[
                LinearSagaEvent::started(&definition, id("inp_start"), id("act_lock")),
                LinearSagaEvent::started(&definition, id("inp_duplicate_start"), id("act_other")),
            ],
        )
        .unwrap_err();
    ...
    fn ordered_replay_rejects_a_sequence_gap_before_applying_events() {
        ...
        let error = replay_ordered(
            ...
            &[LinearSagaEventEnvelope {
                sequence: 1,
                event: LinearSagaEvent::started(&definition, id("inp_start"), id("act_lock")),
            }],
        )
        .unwrap_err();
